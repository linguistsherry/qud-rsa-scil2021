...
pdf("Exp2-qudbias.pdf", height = 1.5, width = 5)
ggplot(summary, aes(x = question, y = rating, fill = question)) +
  geom_bar(position = position_dodge(), stat = "identity") +
  geom_errorbar(aes(ymin = rating - ci, ymax = rating + ci),
                width = 0.2, position = position_dodge(0.9)) +
...
any.min <- lmer(norm ~ qudbias + prior + (1 | subject) + (1 | item), data = res[res$question == "any",],
          control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxfun=1e10)))

any.plus <- lmer(norm ~ qudbias * prior + (1 | subject) + (1 | item), data = res[res$question == "any",],
          control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxfun=1e10)))

summary(glht(any.min, linfct = mcp(prior = "Tukey")))
anova(any.min, any.plus, test = "Chisq")
...
summary(glht(all.min, linfct = mcp(prior = "Tukey")))
anova(all.min, all.plus, test = "Chisq")

many.min <- lmer(norm ~ qudbias + prior + (1 | subject) + (1 | item), data = res[res$question == "many",],
          control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxfun=1e10)))

many.plus <- lmer(norm ~ qudbias * prior + (1 | subject) + (1 | item), data = res[res$question == "many",],
          control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxfun=1e10)))

summary(glht(many.min, linfct = mcp(prior = "Tukey")))
anova(many.min, many.plus, test = "Chisq")
