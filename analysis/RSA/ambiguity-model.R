...
Lexicon <- function(utterance, scope) {
  ...
  else if(utterance == "sbna") {
    Meaning <- sapply(Worlds, function(x) x == 1)
  }
...
### define a listener that ignores the qud

NoQUDLitListener <- function(utterance, scope) {
  interpretation <- Lexicon(utterance, scope) ### get the meaning of the utterance
  output <- interpretation / sum(interpretation) ### normalise
  return(output)
}

NoQUDSpeaker <- function(world, scope, alpha) {
  effects <- sapply(Utterances, function(x) NoQUDLitListener(x, scope)) ### get the listener's interpretation of each utterance
  effects <- effects[rownames(effects)[grepl(world, rownames(effects))],] ### select the row corresponding to the world state
  effects <- exp(alpha * log(effects)) ### softmax
  effects <- effects / sum(effects) ### normalise
  return(effects)
...
NoQUDNewListener <- function(utterance, wpriors, spriors, alpha) {
  ScopePriors <- spriors
  ScopePriors <- ScopePriors / sum(ScopePriors)
  names(ScopePriors) <- Scopes
  ...
  worldOutput <- sapply(Worlds, function(x)
                  sapply(Scopes, function(y)
                      NoQUDSpeaker(x, y, alpha)[utterance] * WorldPriors[as.character(x)] * ScopePriors[y]))
...
### define a listener that ignores prior probabilities

NoPriorNewListener <- function(utterance, spriors, qpriors, alpha) {
  ScopePriors <- spriors
  ScopePriors <- ScopePriors / sum(ScopePriors)
  names(ScopePriors) <- Scopes
  ...
  worldOutput <- sapply(Worlds, function(x)
                  sapply(Scopes, function(y)
                    sapply(QUDs, function(z)
                      ### apparently consideration of priors only occurs here
                      NewSpeaker(x, y, z, alpha)[utterance] * ScopePriors[y] * QUDPriors[z])))

  ### sum over scopes and quds                                                      
  worldOutput <- colMeans(worldOutput)
  names(worldOutput) <- Worlds
  #worldOutput <- worldOutput * Lexicon(utterance, "inverse")
  worldOutput <- worldOutput / sum(worldOutput) ### normalise
